...
- [ ] **➌ Tauri-Commands statt direktem `plugin-sql`-Zugriff** — **in Arbeit seit 2026-06-06**. Foundation gelegt (eigener `sqlx::SqlitePool` im Tauri-State, Migrations selbst via `sqlx::migrate!`, `tauri-plugin-sql` bleibt vorerst als JS-DB-API für die noch nicht portierten Calls). Portiert (7/12): `list_subscriptions`, `list_accounts`, `add_subscription`, `delete_subscription` (mit Transaktion: zuerst Reminders, dann Abo — löst nebenbei das ON-DELETE-CASCADE-Bug), `add_account`, `delete_account`, `count_subs_for_account`. Noch zu portieren (5/12): `updateSubscription`, `insertReminderIfNew`, plus die internen Helper `getDb`, `mapSub`, `parseInterval` werden mit weniger oder gar nicht mehr gebraucht. Wenn alle Frontend-Calls portiert sind, kann `@tauri-apps/plugin-sql` aus dem Frontend raus; die Rust-Crate `tauri-plugin-sql` bleibt dann nur noch, falls wir sie wirklich brauchen (vermutlich nicht). Manuelle Type-Spiegelung (Rust-Struct ↔ TS-Interface, gleicher camelCase via `#[serde(rename_all = "camelCase")]`) statt `tauri-specta` — wenn der Drift-Schmerz real wird, ergänzen wir specta nachträglich.
- [ ] **➋ Reminder-Loop ins Rust-Backend** (hands-on nach ➌-Abschluss). Reminder-Loop im Webview (`App.tsx:55ff`, `setInterval(tick, 60 * 60 * 1000)` in einem `useEffect`). Bekannte Achille-Sehne: pausiert bei minimiertem Fenster / System-Suspend. Echte Architektur-Lösung wäre ein Rust-Side-Scheduler im Hauptprozess. Sobald ➌ vollständig ist, ist der DB-Zugriff aus Rust da und ➋ wird zum Tagesausflug.
