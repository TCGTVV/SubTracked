...
- [ ] **Komponenten-Testbarkeit** — durch ➊ (Custom Hooks, 2026-06-05) und ➌ (Tauri-Commands, 2026-06-06) ist der Pfad jetzt offen: Components rufen Hooks (`useSubscriptions`, `useNotificationPermission`), nicht mehr `db.*` direkt; `db.ts` ist nur noch ein dünner `invoke()`-Wrapper. Für isolierte Hook-Tests reicht `renderHook` mit gemocktem `invoke`. RTL-Adoption für Component-Smoke-Tests fehlt noch (siehe „Komponenten-Tests via React Testing Library" in der Später-Sektion) — Setup deutlich kleiner als vor ➊/➌, aber nicht null: JSDOM, `@testing-library/react`, ein Mock-Helper für `@tauri-apps/api/core`.
