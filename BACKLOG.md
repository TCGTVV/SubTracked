...
- [ ] **➌ Tauri-Commands statt direktem `plugin-sql`-Zugriff** — **in Arbeit seit 2026-06-06**. Foundation gelegt (eigener `sqlx::SqlitePool` im Tauri-State, Migrations selbst via `sqlx::migrate!`, `tauri-plugin-sql` bleibt vorerst als JS-DB-API für die noch nicht portierten Calls). Portiert: `list_subscriptions`, `list_accounts`, `add_subscription`, `delete_subscription` (mit Transaktion: zuerst Reminders, dann Abo — löst nebenbei das ON-DELETE-CASCADE-Bug). Noch zu portieren: `updateSubscription`, `addAccount`, `deleteAccount`, `countSubsForAccount`, `insertReminderIfNew`. Wenn alle 9 portiert sind, kann `@tauri-apps/plugin-sql` aus dem Frontend raus; die Rust-Crate `tauri-plugin-sql` bleibt dann nur noch, falls wir sie wirklich brauchen (vermutlich nicht). Manuelle Type-Spiegelung (Rust-Struct ↔ TS-Interface, gleicher camelCase via `#[serde(rename_all = "camelCase")]`) statt `tauri-specta` — wenn der Drift-Schmerz real wird, ergänzen wir specta nachträglich.
