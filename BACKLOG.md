...
Diskussions-Sektion, **nicht** Hands-on-Task. User hat am 2026-06-05 explizit darum gebeten, weil eine zweite Meinung (Codex) "alles perfekt, keine Luft nach oben" gemeldet hatte. Diese Sektion sammelt die Themen, über die wir uns austauschen sollten, bevor sie Hands-on werden. Erkenntnisse werden danach in konkrete Backlog-Items überführt.

- [ ] **State-Konzept für `App.tsx`**: aktuell 7 `useState` + 3 `useEffect` direkt im Body (211 Zeilen). Hält noch, kippt bei 1–2 weiteren Features. Optionen: Custom Hooks (`useSubscriptions`, `useNotificationPermission`, `useReminderLoop`), Zustand, Jotai, TanStack Query mit DB-Adapter. Auf MVP-Stand bewusst keine Lib, aber das ist eine Entscheidung mit Verfallsdatum.
- [ ] **Backend-Logik im Frontend** (`src/lib/db.ts` enthält die SQL-Queries und das DB↔TS-Mapping, `src-tauri/src/lib.rs` ist quasi leer). Wir nutzen Tauri eher als "Browser mit SQLite-API" als als "Native Backend + UI-Frontend". Alternative: `#[tauri::command]`-Funktionen in Rust, Typen via `tauri-specta` generiert. Trade-off: mehr Boilerplate, klare Schichtung, native Performance bei großen Queries, Frontend wird "dümmer" und einfacher portabel (Mobile später).
- [ ] **Reminder-Loop im Webview** (`App.tsx:55ff`, `setInterval(tick, 60 * 60 * 1000)` in einem `useEffect`). Bekannte Achille-Sehne, schon im HANDOVER vermerkt: pausiert bei minimiertem Fenster / System-Suspend. Echte Architektur-Lösung wäre ein Rust-Side-Scheduler im Hauptprozess. Für eine Notification-zentrale App ist das nicht akademisch.
